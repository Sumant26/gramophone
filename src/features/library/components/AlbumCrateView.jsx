 ...
 * with artwork covers, peeking vinyl discs, mood divider tabs, and liner notes.
 */
export function AlbumCrateView({
  ...
  const [activeDivider, setActiveDivider] = useState('all')

  const dividerTabs = [
    { id: 'all', label: 'All Crates' },
    { id: 'favorites', label: '⭐ Favorites' },
    { id: 'Rainy Days', label: '🌧️ Rainy' },
    { id: 'Cozy Cafe', label: '☕ Cafe' },
    { id: 'Late Night Focus', label: '🌙 Night' },
    { id: 'az', label: '🔤 A–Z' },
  ]
  ...
  // Filter & sort by divider tab
  const albums = useMemo(() => {
    let list = [...rawAlbums]

    if (activeDivider === 'favorites') {
      list = list.filter((a) => a.tracks.some((t) => t.track.isFavorite))
    } else if (
      ['Rainy Days', 'Cozy Cafe', 'Late Night Focus'].includes(activeDivider)
    ) {
      list = list.filter((a) =>
        a.tracks.some(
          (t) =>
            t.track.genre?.toLowerCase().includes(activeDivider.toLowerCase()) ||
            a.title.toLowerCase().includes(activeDivider.toLowerCase()),
        ),
      )
    } else if (activeDivider === 'az') {
      list.sort((a, b) => a.title.localeCompare(b.title))
    }

    return list
  }, [rawAlbums, activeDivider])
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Crate Wooden Divider Tabs & Custom Pressing Button */}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-cozy-brass/15 pb-2">
        <div
          role="tablist"
          aria-label="Crate divider tabs"
        ...
        >
          {dividerTabs.map((tab) => (
            <button
              ...
              type="button"
              role="tab"
              aria-selected={activeDivider === tab.id}
              onClick={() => setActiveDivider(tab.id)}
              className={clsx(
                'rounded-lg px-2.5 py-1 text-xs font-serif-display transition-all shadow-sm',
                activeDivider === tab.id
                  ? 'bg-cozy-brass font-bold text-cozy-on-accent'
                  : 'bg-cozy-surface-2 text-cozy-ink-muted hover:text-cozy-ink hover:bg-cozy-brass/10 border border-cozy-brass/15',
              )}
            >
              {tab.label}
            </button>
        ...
        {onOpenMixtape && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-full bg-cozy-surface-2 border border-cozy-brass/30 px-3 py-1 text-xs font-semibold text-cozy-brass-light hover:bg-cozy-brass hover:text-cozy-on-accent transition-colors shadow-sm"
          >
            <span>📀</span>
            <span>Press Mixtape</span>
          </button>
        )}
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-1 flex-col justify-between p-4">
                ...
                {/* Action Buttons */}
                <div className="mt-3 flex items-center justify-between gap-1.5 border-t border-cozy-brass/15 pt-3">
                  <button
                    type="button"
                    onClick={() =>
                      onPlayTrack(album.tracks[0].track, album.tracks[0].originalIndex)
                  ...
                  {onOpenGatefold && (
                    <button
                      type="button"
                      onClick={() => onOpenGatefold(album)}
                      className="flex items-center gap-1 text-xs text-cozy-ink-muted hover:text-cozy-brass-light transition-colors"
                      title="View Gatefold Sleeve & Liner Notes"
                    >
                  ...
                  >
                    <span>{isExpanded ? 'Hide' : 'Songs'}</span>
                    <span
                      className={clsx(
